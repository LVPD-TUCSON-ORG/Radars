...
local checkDistance = 15.0

function main()
    sampRegisterChatCommand('radarssf', function()
        toggleRadars('SF', coordsSF)
    end)
    
    sampRegisterChatCommand('radarsls', function()
        toggleRadars('LS', coordsLS)
    end)
    
    sampRegisterChatCommand('radarslv', function()
        toggleRadars('LV', coordsLV)
    end)
    
    while true do
        wait(100)
        checkPlayerDistanceToBlips()
    end
end

function toggleRadars(city, coordsTable)
    if currentCity == city then
        removeAllBlips()
        currentCity = nil
    else
        removeAllBlips()
        createBlips(coordsTable)
        currentCity = city
    end
end

function createBlips(coordsTable)
...
function checkPlayerDistanceToBlips()
    ...
    for blipId, coord in pairs(activeBlips) do
        local distance = getDistanceBetweenCoords3d(vehX, vehY, vehZ, coord.x, coord.y, coord.z)
