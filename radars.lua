	...
	if radars == false then
		...
		lua_thread.create(function()
			for _, coord in ipairs(coordsTable) do
				--table.insert(checkpoints, createCheckpoint(2, coord.x, coord.y, coord.z + 1.5, 0, 0, 0, 1.0))
				print(coord.x, coord.y, coord.z)
				local blip = addSpriteBlipForCoord(coord.x, coord.y, coord.z + 1.5, 56)
				--placeWaypoint(coord.x, coord.y, coord.z)
				if doesBlipExist(blip) then
					table.insert(blips, blip)

					--changeBlipColour(blip, 0xFFFFFF) -- 15 - MinJustice
				else
